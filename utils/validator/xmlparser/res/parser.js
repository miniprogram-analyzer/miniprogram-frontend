...
({
  ...
  "./src/nimndata.js":
    ...
    (function (module, exports, __webpack_require__) {
      ...
      var _e = function _e(node, e_schema, options) {
        if (typeof e_schema === 'string') { //premitive
          ...
          if (hasValidData === true) {
            var str = '';
            if (Array.isArray(e_schema)) { //attributes can't be repeated. hence check in children tags only
              str += chars.arrStart;
      ...
      var x2j = __webpack_require__( /*! ./xmlstr2xmlnode */ "./src/xmlstr2xmlnode.js");
      var buildOptions = __webpack_require__( /*! ./util */ "./src/util.js").buildOptions;
          ...
          if (xmlNode.child) {
            ...
            Object.keys(xmlNode.child).forEach(function (key) {
              ...
              if (Array.isArray(node)) {
                ...
                node.forEach(function (item, index) { //console.log(indentation + " \""+index+"\" : [")
  ...
  "./src/validator.js":
    ...
    (function (module, exports, __webpack_require__) {
      ...
      exports.validate = function (xmlData, options) {
        ...
        for (var i = 0; i < xmlData.length; i++) {
          if (xmlData[i] === '<') { //starting of tag
            ...
            if (xmlData[i] === '?') {
              ...
              if (!validateTagName(tagName)) {
                ...
                if (tagName.trim().length === 0) {
                  ...
                  msg = "标签 '" + tagName + "' 不是一个有效的名称.";
                }
                return getErrorObject('无效的标签', msg, getLineNumberForPosition(xmlData, i));
              }
              var result = readAttributeStr(xmlData, i);
              if (result === false) {
                return getErrorObject('无效的属性', "属性'" + tagName + "'的值应当使用引号.", getLineNumberForPosition(xmlData, i));
              }
              ...
              if (attrStr[attrStr.length - 1] === '/') { //self closing tag
                ...
                if (!result.tagClosed) {
                  return getErrorObject('无效的标签', "闭合标签 '" + tagName + "' 未正确闭合.", getLineNumberForPosition(xmlData, i));
                } else if (attrStr.trim().length > 0) {
                  return getErrorObject('无效的标签', "闭合标签 '" + tagName + "' 不应该有属性或开头非法.", getLineNumberForPosition(xmlData, i));
                } else {
                  var otg = tags.pop();
                  if (tagName !== otg) {
                    return getErrorObject('无效的标签', "闭合标签 '" + otg + "' 不应出现，应当为 '" + tagName + "'.", getLineNumberForPosition(xmlData, i));
              ...
              for (i++; i < xmlData.length; i++) {
                if (xmlData[i] === '<') {
                  ...
                  if (afterAmp == -1) return getErrorObject('无效的字符', "字符 '&' 不应当出现.", getLineNumberForPosition(xmlData, i));
                  i = afterAmp;
                }
            ...
            return getErrorObject('无效的字符', "字符 '" + xmlData[i] + "' 不应当出现.", getLineNumberForPosition(xmlData, i));
          }
        }
        if (!tagFound) {
          return getErrorObject('无效的WXML', '需要开始标签.', 1);
        } else if (tags.length > 0) {
          return getErrorObject('无效的WXML', "发现无效的 '" + JSON.stringify(tags, null, 4).replace(/\r?\n/g, '') + "'.", 1);
        }
        return true;
      };
      /**
      ...
      function readPI(xmlData, i) {
        ...
        for (; i < xmlData.length; i++) {
          if (xmlData[i] == '?' || xmlData[i] == ' ') { //tagname
            ...
            if (i > 5 && tagname === 'xml') {
              return getErrorObject('无效的WXML', 'WXML无需XML声明.', getLineNumberForPosition(xmlData, i));
      ...
      function validateAttributeString(attrStr, options) { //console.log("start:"+attrStr+":end");
        ...
        for (var i = 0; i < matches.length; i++) {
          if (matches[i][1].length === 0) { //nospace before attribute name: a="sd"b="saf"
            return getErrorObject('无效的属性', "属性 '" + matches[i][2] + "' 开头不应当有空格.", getPositionFromMatch(attrStr, matches[i][0]));
          } else if (matches[i][3] === undefined && !options.allowBooleanAttributes) { //independent attribute: ab
            return getErrorObject('无效的属性', "不允许布尔值类型 '" + matches[i][2] + "'.", getPositionFromMatch(attrStr, matches[i][0]));
          }
          ...
          if (!validateAttrName(attrName)) {
            return getErrorObject('无效的属性', "属性 '" + attrName + "' 名称无效.", getPositionFromMatch(attrStr, matches[i][0]));
          }
          if (!attrNames.hasOwnProperty(attrName)) { //check for duplicate attribute.
            attrNames[attrName] = 1;
          } else {
            return getErrorObject('无效的属性', "属性 '" + attrName + "' 重复.", getPositionFromMatch(attrStr, matches[i][0]));
          }
        }
        return true;
      }
      ...
      function validateAmpersand(xmlData, i) { // https://www.w3.org/TR/xml/#dt-charref
        i++;
