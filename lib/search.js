...
import waves from "/vue/directive/waves/index.js";
  ...
  methods: {
      ...
      API.getdebugslist({})
        .then((res) => {
          ...
          for (var i = 0; i < this.questions.length; i++) {
            var tag = this.questions[i].tag.replace(/\"/g, "").trim();
            this.questions[i].tag = tag.trim().replace(/\'/g, '"');
            tag = JSON.parse(this.questions[i].tag);
            this.questions[i].tag = tag;
          }
        })
        .catch((err) => {
          console.log(err);
    ...
    querySearchAsync(queryString, cb) {
      ...
      var results = queryString ?
        questions.filter(this.createStateFilter(queryString)) :
        questions;
    ...
    createStateFilter(queryString) {
      ...
      */
      return (state) => {
        ...
        return (
          state.detail.toLowerCase().indexOf(queryString.toLowerCase()) !== -1
        );
      };
    },
    // 搜索键用的函数，对应第二条支线
