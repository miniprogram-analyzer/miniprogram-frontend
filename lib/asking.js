...
var vm = new Vue({
  ...
  data: {
    ...
    options: [{value:"页面设计",label:"页面设计: 通过wxml进行页面设计"},         //wxml
              {value:"样式美化",label:"样式美化: 通过wxss进行页面美化"},         //wxss
              {value:"功能逻辑",label:"功能逻辑: JavaScript语言逻辑实现"},  //js
              {value:"文件结构",label:"文件结构: 微信小程序文件结构"},         //framework
              {value:"组件",label:"组件: 微信小程序自带组件使用"},                //component
              {value:"API调用",label:"API调用: 微信API调用方式"},          //api
              {value:"云开发",label:"云开发: 云函数开发与云数据库调用等问题"},            //wxcloud
      ...
      if (!loading) {
        ...
        $.ajax({
          ...
          data: {
            ...
            partition: classify,
            title: question,
            guestid: window.localStorage.getItem("user_id"),
