...
var load_json = function (result) {
  ...
  for (var i = 0; i < miniprogramCom.length; i++) {
    if(hasComponent[i] != 0){
      ...
      for (var j = 0; j < miniprogramCom[i].children.length; j++) {
        if(miniprogramCom[i].children[j].count != 0) {
  ...
  var count = 0
  for (let key in result.wxAPIs) {
    miniprogramAPIs += "<tr><td>" + (++count) + "</td><td><a target=\"_blank\" href='https://developers.weixin.qq.com/doc/search.html?doc_type=miniprogram&query=" + key + "'>" + key + "</a></td><td>" + result.wxAPIs[key] + "&nbsp;次</td></tr>"
  }
  miniprogramAPIs += "</tbody>"
  jsonResult = result
  $("#miniprogram-apis").html(miniprogramAPIs);
}

const local_result = window.localStorage.getItem("analysis_current")
if(location.search !== null && location.search !== undefined && location.search !== '') {
  $.getJSON("/reports/" + location.search.split("report=")[1] +"/report.json", function(result){load_json(result)}).catch(function(err){window.location.replace("/404");});
} else if (local_result !== null && local_result !== undefined) {
  load_json(JSON.parse(local_result))
} else {
  var analyze_history = window.localStorage.getItem('analyze_history')
  if(analyze_history !== null && analyze_history !== undefined && analyze_history !== ''){
    analyze_history = JSON.parse(analyze_history)
    console.log()
    $.getJSON(analyze_history[analyze_history.length-1].report_path.replace(/\/data\/miniprogram-analyzer/,"") +"/report.json", function(result){load_json(result)}).catch(function(err){window.location.replace("/404");});
  }
  else {
    $("#content").html('<div class="empty"><img src="/img/analysis.png" /><p>尚未进行过分析，<a href="/analysing">点此进行</a></p></div>');
  }
}