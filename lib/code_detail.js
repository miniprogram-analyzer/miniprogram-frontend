...
$.getJSON("/report/seed/report.json", function (result) {
    console.log(result)
                ...
                if (document.querySelector("div#mask") === null) {
                    ...
                    for (var j = 0; j < result.plato.length; j++) {
                        if (result.plato[j].info.file.indexOf(page) > 0) {
                            var title = "<div class=\"bb code-detail\"><span style=\"margin:15px 0 0;cursor:auto;border-bottom:2px solid #ec672f;word-break:keep-all;text-overflow:ellipsis;overflow:hidden;max-width:calc(94vw - 100px)\">页面\"" + page + "\"详细信息</span></div>"
                            ...
                            content += "</div>"
                            popWin.showDetail(title, content /*result.plato[j]*/ );
                            break;
                        }
                    }
                }
