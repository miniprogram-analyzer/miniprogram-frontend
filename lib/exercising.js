...
$.ajax({
    ...
    success: function (res) {
        ...
        for (var i=0;i<res.msg.length;i++){
            problemHTML += "<tr><td>P" + res.msg[i].serial + "</td><td><a href=\"/problem?id=" + res.msg[i].serial + "\">" + (res.msg[i].title || '问题' + res.msg[i].serial) + "</a></td><td>" + (res.msg[i].submit || 0) + "</td><td>" + (res.msg[i].accept || 0) + "</td><td>" + (res.msg[i].difficulty || 0) + "</td></tr>"
        }
        problemHTML += "</tbody>"
        $("#problems-list").html(problemHTML);
    },
